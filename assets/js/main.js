...
(function(){
  ...
  // ---------------------------------------------------------------------
  // 3️⃣  Load content JSON for the selected language (fallback to zh‑Hant)
  // ---------------------------------------------------------------------
  const loadContent = (lang = 'zh-Hant') => {
    ...
    const promises = Object.entries(files).map(([key, file]) => {
      return fetch(`${base}/${file}`)
        .then(r => r.ok ? r.json() : Promise.reject(`${file} missing`))
        .then(json => ({ key, html: json.html || "" }))
        .catch(() => ({ key, html: "" }));
    });
    Promise.all(promises).then(items => {
      items.forEach(({ key, html }) => {
        const el = document.getElementById(`${key}-cn`);
